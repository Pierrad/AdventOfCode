...
const isRangeOverlapWithAnother = (range, anotherRange) => {
  return range.some((number) => anotherRange.includes(number)) || anotherRange.some((number) => range.includes(number));
};
...
const main = () => {
  ...
  for (let i = 0; i < explicitAssignmentPairs.length; i++) {
    ...
    if (isRangeOverlapWithAnother(range, anotherRange)) {
